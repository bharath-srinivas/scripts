...
#Download the backup into the specific folder
function download(){
	...
	if [[ -f "SM_Website_live_$NOW.tar.gz" && -f "SM_Website_DB_$NOW.sql" ]]; then
		is_success=true
	fi

	#Move the new backup files to the root of web.txtbox.in directory
	for j in "${FILES[@]}"; do
		echo "magics3rv3r" | sudo mv $j /var/www/web.txtbox.in/website_bckps
	done
...
function success(){
	if [ "$is_success" == true ]; then
		echo "The backup download process has been completed successfully"
	else
